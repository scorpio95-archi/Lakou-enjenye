...
const DISCIPLINE_SLUG = 'genie-informatique';
...
const filterStructure = document.getElementById('filter-structure'); // ici : type de projet
...
async function init() {
  ...
  if (discErr || !discipline) {
    showError("Impossible de charger la discipline Génie Informatique.");
  ...
  const { data: projects, error: projErr } = await supabase
    ...
    .select(`
      ...
      specs_informatique ( type_projet, stack_technique, repo_url, demo_url, architecture_diagram_url )
    `)
...
function updateCount(n) {
  projectCount.textContent = `${n} projet${n > 1 ? 's' : ''} validé${n > 1 ? 's' : ''} — GÉNIE INFORMATIQUE`;
}
  ...
  list.forEach(project => {
    ...
    card.innerHTML = `
      ...
      <div class="card-body">
        ...
        <div class="card-meta">
          <span>${project.specs_informatique?.[0]?.type_projet || '—'}</span>
...
function applyFilters() {
  ...
  const filtered = allProjects.filter(p => {
    ...
    const matchStructure = !structureVal || p.specs_informatique?.[0]?.type_projet === structureVal;
...
async function openModal(project) {
  ...
  const spec = project.specs_informatique?.[0] || {};
  document.getElementById('spec-type-projet').textContent = spec.type_projet || '—';

  // Stack technique affichée en tags, pas en texte brut
  const stackContainer = document.getElementById('spec-stack');
  stackContainer.innerHTML = '';
  if (spec.stack_technique && spec.stack_technique.length > 0) {
    spec.stack_technique.forEach(tech => {
  ...
  const repoLink = document.getElementById('spec-repo-link');
  if (spec.repo_url) {
    repoLink.href = spec.repo_url;
    repoLink.style.opacity = '1';
  } else {
    repoLink.removeAttribute('href');
    repoLink.style.opacity = '0.4';
  }

  const demoLink = document.getElementById('spec-demo-link');
  if (spec.demo_url) {
    demoLink.href = spec.demo_url;
    demoLink.style.opacity = '1';
  } else {
    demoLink.removeAttribute('href');
    demoLink.style.opacity = '0.4';
  }

  const diagramLink = document.getElementById('spec-diagram-link');
  if (spec.architecture_diagram_url) {
    diagramLink.href = spec.architecture_diagram_url;
    diagramLink.style.opacity = '1';
  } else {
    diagramLink.removeAttribute('href');
    diagramLink.style.opacity = '0.4';
  }
