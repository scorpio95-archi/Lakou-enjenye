...
const DISCIPLINE_SLUG = 'electromecanique';
...
const filterStructure = document.getElementById('filter-structure'); // ici : matériau principal
...
async function init() {
  ...
  if (discErr || !discipline) {
    showError("Impossible de charger la discipline Électromécanique.");
  ...
  const { data: projects, error: projErr } = await supabase
    ...
    .select(`
      ...
      specs_electromecanique ( materiau_principal, puissance_moteur_kw, temperature_max_c, norme_appliquee, modele_3d_url )
    `)
...
function updateCount(n) {
  projectCount.textContent = `${n} projet${n > 1 ? 's' : ''} validé${n > 1 ? 's' : ''} — ÉLECTROMÉCANIQUE`;
}
  ...
  list.forEach(project => {
    ...
    card.innerHTML = `
      ...
      <div class="card-body">
        ...
        <div class="card-meta">
          <span>${project.specs_electromecanique?.[0]?.materiau_principal || '—'}</span>
...
function applyFilters() {
  ...
  const filtered = allProjects.filter(p => {
    ...
    const matchStructure = !structureVal || p.specs_electromecanique?.[0]?.materiau_principal === structureVal;
...
async function openModal(project) {
  ...
  const spec = project.specs_electromecanique?.[0] || {};
  document.getElementById('spec-materiau').textContent = spec.materiau_principal || '—';
  document.getElementById('spec-puissance').textContent = spec.puissance_moteur_kw ? `${spec.puissance_moteur_kw} kW` : '—';
  document.getElementById('spec-temperature').textContent = spec.temperature_max_c ? `${spec.temperature_max_c} °C` : '—';
  document.getElementById('spec-norme').textContent = spec.norme_appliquee || '—';

  const modeleLink = document.getElementById('spec-modele-link');
  if (spec.modele_3d_url) {
    modeleLink.href = spec.modele_3d_url;
    modeleLink.style.opacity = '1';
  } else {
    modeleLink.removeAttribute('href');
    modeleLink.style.opacity = '0.4';
  }
